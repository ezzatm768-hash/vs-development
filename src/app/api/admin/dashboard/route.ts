...
export async function GET(req: Request){
  ...
  const reportsByTeamLeader=db.users.filter((u:any)=>u.role==="team_leader").map((l:any)=>{
    const mine=db.evaluations.filter((e:any)=>e.team_leader_id===l.id || e.team_leader_id==="u2" && l.id==="u2" || e.team_leader_id==="2" && l.id==="u2");
  ...
  // حساب حالة كل Sales للـ Admin overview
