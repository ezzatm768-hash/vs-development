...
export async function GET(req: Request, { params }: { params: Promise<{ id: string }> }){
  ...
  if(convex){
    try{
      ...
      if(callerId){ const data = await convex.query("evaluations:getById" as any, { callerId, id }); return NextResponse.json(data); }
    }catch(e: any){ return NextResponse.json({error:String(e?.message||"غير موجود")},{status:404}); }
  }
