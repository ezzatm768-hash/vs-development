...
export async function PUT(req: Request, { params }: { params: Promise<{ id: string }> }){
  ...
  if(convex && isConvexId(user.id)){
    try{
      await convex.mutation("sales:update" as any, { callerId: user.id, id, name: body.name, phone: body.phone, join_date: body.join_date });
...
export async function DELETE(req: Request, { params }: { params: Promise<{ id: string }> }){
  ...
  if(convex && isConvexId(user.id)){
    try{
      await convex.mutation("sales:remove" as any, { callerId: user.id, id });
